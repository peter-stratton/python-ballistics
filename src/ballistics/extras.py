...
def approximate_free_recoil_energy(firearm_weight_in_lbs, ejecta_weight_in_grains, charge_weight_in_grains,
    ...
    -------
    int or float
        How many foot-pounds of force (ft.lbf) are generated by the input parameters.  By default ``decimal_places`` is 0 which
...
def approximate_recoil_velocity(firearm_weight_in_lbs, ejecta_weight_in_grains, muzzle_velocity_in_fps,
    ...
    -------
    int or float
        Velocity in feet per second (fps) of the firearms recoil.  By default ``decimal_places`` is 0 which will return
        the nearest integer value.  If a value greater than 0 is passed in, the function will return a float value
